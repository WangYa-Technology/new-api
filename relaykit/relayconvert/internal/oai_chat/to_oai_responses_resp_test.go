...
func TestChatCompletionsStreamUsageTextIncludesReopenedMessageSegments(t *testing.T) {
	state := NewChatToResponsesStreamState("resp_1", "gpt-test")
	firstFinish := "tool_calls"
	secondFinish := "stop"

	chunks := []*dto.ChatCompletionsStreamResponse{
		{Choices: []dto.ChatCompletionsStreamResponseChoice{{Delta: dto.ChatCompletionsStreamResponseChoiceDelta{Content: lo.ToPtr("first")}}}},
		{Choices: []dto.ChatCompletionsStreamResponseChoice{{FinishReason: &firstFinish}}},
		{Choices: []dto.ChatCompletionsStreamResponseChoice{{Delta: dto.ChatCompletionsStreamResponseChoiceDelta{Content: lo.ToPtr("second")}, FinishReason: &secondFinish}}},
	}
	for _, chunk := range chunks {
		mustResponsesEventsFromChatChunk(t, state, chunk)
	}

	assert.Equal(t, "firstsecond", state.UsageText())
}
