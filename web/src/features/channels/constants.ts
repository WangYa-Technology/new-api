...
export const CHANNEL_TYPE_SUB2API = 60

export const CHANNEL_TYPE_NEW_API = 61

export const CHANNEL_TYPE_HCAI = 62

export const CHANNEL_TYPE_KEYIYUN = 63

export const CHANNEL_TYPE_KEMEI = 64
...
> = {
  ...
  59: { descriptionKey: 'Connect to model services through Jiekou Seedance' },
  60: { descriptionKey: 'Connect to model services through a Sub2API gateway' },
  61: {
    descriptionKey:
      'Connect to New API model services with support for multiple task plugins',
  },
  62: { descriptionKey: 'Connect to HCAI model services' },
  63: { descriptionKey: 'Connect to Keyiyun model services' },
  64: { descriptionKey: 'Connect to Kemei AI model services' },
...
const CHANNEL_TYPE_DISPLAY_ORDER: number[] = [
  ...
  5, 36, 50, 51, 52, 53, 54, 55, 56, 59, 60, 62, 63, 64, 65, 66, 67,
]

// Retained for database compatibility, but no longer offered as new channels.
export const RETIRED_CHANNEL_TYPES = new Set([59, 62, 63, 64])

export const CHANNEL_TYPE_OPTIONS: { value: number; label: string }[] = (() => {
  const ordered: { value: number; label: string }[] = []
  const seen = new Set<number>()
  for (const id of CHANNEL_TYPE_DISPLAY_ORDER) {
    if (RETIRED_CHANNEL_TYPES.has(id)) continue
  ...
  for (const [key, label] of Object.entries(CHANNEL_TYPES)) {
    const id = Number(key)
    if (id !== 0 && !RETIRED_CHANNEL_TYPES.has(id) && !seen.has(id)) {
      ordered.push({ value: id, label })
    }
  }
  return ordered
