...
func (a *TaskAdaptor) ParseResponse(_ *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*channel.TaskSubmitResponse, *taskdto.TaskError) {
	...
	if err := common.Unmarshal(responseBody, &result); err != nil {
		return nil, service.TaskErrorWrapper(fmt.Errorf("unmarshal Keyiyun response: %w", err), "unmarshal_response_body_failed", http.StatusBadGateway)
	}
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return nil, service.TaskErrorWrapper(fmt.Errorf("Keyiyun API error: %s", responseMessage(result, resp.StatusCode)), "keyiyun_api_error", resp.StatusCode)
	}
	if strings.TrimSpace(result.ID) == "" {
		return nil, service.TaskErrorWrapper(fmt.Errorf("Keyiyun task id is empty"), "invalid_response", http.StatusBadGateway)
	}
	...
	return &channel.TaskSubmitResponse{UpstreamTaskID: result.ID, TaskData: responseBody, ClientResponse: video}, nil
}
